...
const normalizePath = filepath => path.relative(process.cwd(), filepath);
...
var postcssLoader = {
  ...
  process({ code, map }) {
    return new Promise(function ($return, $error) {
      var config, options, plugins, shouldExtract, shouldInject, modulesExported, postcssOpts, res, json, getClassName, newName;
...
var stylusLoader = {
  ...
  process({ code }) {
    return new Promise(function ($return, $error) {
      ...
      stylus = localRequire('stylus');
      style = stylus(code).set('filename', this.id).set('sourcemap', this.sourceMap && {});
...
var lessLoader = {
  ...
  process({ code }) {
    return new Promise(function ($return, $error) {
      var less, css, map;
      less = localRequire('less');
      return Promise.resolve(pify(less.render.bind(less))(code, {
        sourceMap: this.sourceMap && {}
      })).then(function ($await_1) {
        try {
          ({ css, map } = $await_1);
...
var index = ((options = {}) => {
  ...
  return {
    ...
    onwrite(opts) {
      return new Promise(function ($return, $error) {
        var getExtracted, shouldExtract, code, codeFilePath, map, mapFilePath;
        ...
        getExtracted = filepath => {
          ...
          for (const res of extracted.values()) {
            ...
            if (map) {
              map.file = filepath;
              map.sources = map.sources.map(source => normalizePath(path.join(path.dirname(opts.file), source)));
            }
        ...
        if (options.onExtract) {
          return Promise.resolve(options.onExtract(getExtracted)).then(function ($await_3) {
            try {
              shouldExtract = $await_3;
        ...
        function $If_1() {
          ({ code, codeFilePath, map, mapFilePath } = getExtracted());
          return Promise.resolve(fs.ensureDir(path.dirname(codeFilePath)).then(() => Promise.all([fs.writeFile(codeFilePath, code, 'utf8'), sourceMap === true && fs.writeFile(mapFilePath, map, 'utf8')]))).then(function ($await_4) {
            try {
              return $return();
            } catch ($boundEx) {
              return $error($boundEx);
